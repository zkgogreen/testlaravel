            ...
            if (data) {
                ...
                densitymap = L.geoJSON(data, {
                    style: function(feature) {
                        ...
                        var fillColor = getColor(density);
                        return {
                            fillColor: fillColor,
                            weight: 1,
                            opacity: 1,
...
//  function setParentMeasure(el, newParent)
//  {
//     newParent.appendChild(el);
//  }
//  setParentMeasure(htmlObject, a);
...
<script>
    var {{$item->title}} = new L.GeoJSON.AJAX("{{ url('getMarkerData/' . $item->title) }}", {
        pointToLayer: function (feature, latlng) {
            var marker = L.marker(latlng, {
                icon: IPB1
            });
        ...
        onEachFeature: function (feature, layer) {
                if (feature.properties) {
                    var content = "<div class='table-responsive'><table class='table' style='margin-bottom:0;' id='tabelMarker'>";
                    $.each(feature.properties, function (key, value) {
                        content += '<tr><th style="width:30%;">' + key + '</th><td>' + value + '</td></tr>';
                    });
                    content += "</table></div>";
                    layer.on('click', function (e) {
                        $("#drag_title_peta").html({{$item->title}});
                        $("#feature-info").html(content);
                        $("#btnmodalpeta").trigger("click");
                    });
                }
